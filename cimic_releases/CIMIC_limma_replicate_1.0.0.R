...
#' (which shrinks n and discards within-cell-line variance) or treating the
#' replicates as independent (pseudoreplication), this fits the moderated
...
#'
#' This preserves the replicate-level degrees of freedom (higher power) yet
#' does not double-count correlated replicates. 2 groups -> moderated t
#' (effect = |t|); >=3 groups -> moderated F (effect = F). adj_p_value is the
#' BH FDR. Returns the FULL per-gene table; the shared caller applies
#' p<=0.05 & adj<=thresh.
#'
