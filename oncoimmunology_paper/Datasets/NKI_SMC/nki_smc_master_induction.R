# =============================================================================
# MAKE MASTER INDUCTION DATAFRAME DIRECTLY
# =============================================================================
# Purpose:
#   Create the NKI master-induction dataframe directly from the final
...
#   - LIMMA robust + trend
#   - No duplicateCorrelation because NKI is a patient cohort
...
# -----------------------------------------------------------------------------
# 2. DEFINE YOUR INPUT DATAFRAME
# -----------------------------------------------------------------------------
# Change this ONLY if your clustered dataframe has a different name.


# file location
clustered_csv_location <- "D:/OneDrive - University of Florida/Gbadamosi Lab/Mohammed Gbadamosi/Gbadamosi_Lab_GitHub/CIMIC/oncoimmunology_paper/Results/nki_smc_cimic_results/CIM_states_results_hc_pacmap_app_three/clustered_samples_app_three.csv"
...
# output directory for saving the master induction dataframe
output_dir <- "D:/OneDrive - University of Florida/Gbadamosi Lab/Mohammed Gbadamosi/Gbadamosi_Lab_GitHub/CIMIC/oncoimmunology_paper/Datasets/NKI_SMC"
...
cat("Clusters:",
    paste(sort(unique(clustered_df$cluster_assignments)), collapse = ", "),
    "\n")
...
# 7. RUN LIMMA
# -----------------------------------------------------------------------------
# NKI is a patient cohort, so there is NO duplicateCorrelation block.
#
# This reproduces:
#   lmFit()
#   eBayes(robust = TRUE, trend = TRUE)
#
# used in the downstream library.
...
# -----------------------------------------------------------------------------
# For two clusters, this is the moderated t-test p-value.
#
# For >2 clusters, this would be the omnibus LIMMA F-test.
...
# Make absolutely sure genes remain in the original order
tt <- tt[
...
# -----------------------------------------------------------------------------
# IMPORTANT:
# H_value is intentionally NOT replaced by LIMMA.
#
# For two clusters:
#   H_value = Wilcoxon/Mann-Whitney W statistic
#
# This preserves compatibility with downstream rank-biserial correlation code.
...
#   SD
#   SEM
#   penetrance = proportion of samples with induction > 0
...
# -----------------------------------------------------------------------------
# For two clusters this creates:
#
#   cluster1_cluster2_differential
#   cluster2_cluster1_differential
#   cluster1_cluster2_sd
#   cluster2_cluster1_sd
#
# This preserves the structure expected by your downstream scripts.
...
if (n_clusters >= 2) {
    ...
    for (a in 1:(n_clusters - 1)) {
        ...
        for (b in (a + 1):n_clusters) {
            ...
            # Extract per‑cluster statistics using proper double‑bracket indexing.
...
if (n_clusters == 2) {
    ...
    master_induction_df$limma_test <-
        "limma_2grp(mod_t)"
...
#
  ...
  csv_file_master_induction <- file.path(output_dir, "nki_smc_master_induction_df.csv")
  fwrite(master_induction_df, csv_file_master_induction)

  csv_file_cluster_induction <- file.path(output_dir, "nki_smc_combine_clustered_plot_df.csv")
