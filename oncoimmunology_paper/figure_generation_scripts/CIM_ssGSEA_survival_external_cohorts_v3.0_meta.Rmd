---
title: "CIM ssGSEA Adjusted Survival Analysis: Meta-Analysis Signatures"
author: "Gbadamosi Lab - IL/MG"
date: "`r format(Sys.Date(), '%B %d, %Y')`"
# knitr would otherwise use this file's own folder as the working directory,
# which would break the repo-relative paths in params. "../.." walks from
# oncoimmunology_paper/figure_generation_scripts/ up to the repository root.
knit_root_dir: "../.."
output:
  html_document:
    toc: true
...
params:
  ...
  # left 08_KM_Plots empty. Requires the survminer package and "median_split" in
  # score_parameterizations; both are satisfied above.
...
Kaplan-Meier curves are descriptive only and never contribute to inference. They
are enabled in v3.0 with `run_km_median_split: true`; set it to `false` to skip
them and leave `08_KM_Plots` empty.
...
# wrong wd here with one clear message rather than five missing-file paths. When
# knitting, the working directory is the folder containing the Rmd, so
# knit_root_dir must be set to the repository root (see the note below).
if (!dir.exists(data_dir)) {
  stop("data_dir does not exist: ", data_dir,
       "\n  params$data_dir = ", params$data_dir,
       "\n  working directory = ", getwd(),
       if (!is_absolute_path(params$data_dir))
         paste0("\n  data_dir is relative. When knitting, set the knit root to ",
                "the repository root (RStudio: Knit > Knit Directory > ",
                "Project Directory, or add knit_root_dir to the YAML). ",
                "Otherwise set params$data_dir to an absolute path.")
       else "")
}
