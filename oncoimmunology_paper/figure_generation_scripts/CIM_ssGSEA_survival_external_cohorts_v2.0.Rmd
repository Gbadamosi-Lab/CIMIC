---
title: "CIM ssGSEA Adjusted Survival Analysis: Continuous and Median-Split Scores"
...
output:
  html_document:
    ...
    theme: readable
params:
  data_dir: "C:/Users/yasmi/OneDrive - University of Florida/Gbadamosi Lab/Iasmim Lopes/01.CIMIC/CIM_ssGSEA/data"
  output_dir: "CIM_ssGSEA_survival_adjusted_cont_median_corr060"
  signature_file: "nki_smc_correlation_analysis.csv"
  metabric_clinical_file: "METABRIC_clinical_data.xlsx"
  metabric_expression_file: "METABRIC_expression_data.csv"
  scanb_clinical_file: "SCANB_clinical_harmonized_OS_DRFi_RFi.rds"
  scanb_expression_file: "SCANB_expression_harmonized_OS_DRFi_RFi.rds"
  ...
  chemo_positive_values: ["true", "1", "yes", "y", "treated", "chemotherapy"]
  # Correlation-file gene-selection settings.
  correlation_threshold: 0.60
  correlation_operator: ">="
  fdr_threshold: 0.05
  fdr_operator: "<"
  # Optional manuscript-version safeguards. Set either value to null when
  # intentionally using another threshold or correlation-file version.
  expected_fun_cim_genes: 542
  expected_dys_cim_genes: 269
  ...
  score_parameterizations: ["continuous", "median_split"]
  m2_covariates: ["age_5y", "tumor_size", "grade", "clinical_subtype"]
  ...
  km_break_months: 48
  metabric_km_ylim: [0.00, 1.00]
  scanb_km_ylim: [0.75, 1.00]
  metabric_km_pval_coord: [0.15, 0.20]
  scanb_km_pval_coord: [0.15, 0.80]
  km_width: 7
  km_height: 6.6
  km_dpi: 600
  seed: 2026
  ...
  hr_axis_min: 0.62
  hr_axis_max: 1.65
  hr_axis_breaks: [0.65, 0.80, 1.00, 1.25, 1.60]
  forest_width: 7.2
  forest_height: 3.8
...
# During knitting, rmarkdown creates `params` from the YAML header. During
# interactive chunk execution, an older `params` object can remain in the
# Global Environment. Reload the active RMD YAML in that situation so edited
# correlation and FDR settings take effect.
...
## Correlation-based signature definition

The correlation CSV is filtered entirely from the active YAML settings. A
positive `r_rb` defines Fun-CIM and a negative `r_rb` defines Dys-CIM. The
optional input `trajectory` column is retained for audit only and does not
control selection. With `correlation_operator: ">="`, the effective rules are
`r_rb >= correlation_threshold` for Fun-CIM and
`r_rb <= -correlation_threshold` for Dys-CIM. The correlation-file `padj` must
also satisfy the user-selected `fdr_operator` and `fdr_threshold`.

Run different cutoffs in separate folders. For example, use
`output_dir: "CIM_ssGSEA_survival_results_corr050"` with
`correlation_threshold: 0.50`, and use the `corr060` folder with a threshold of
`0.60`. If a folder ends in `corrNNN`, the RMD verifies that the suffix and
threshold agree before creating outputs.

# Packages, paths, and analysis configuration

```{r packages-paths-configuration}
required_packages <- c(
...
correlation_threshold <- validate_numeric(
  params$correlation_threshold, "params$correlation_threshold", 0, 1)
correlation_operator <- validate_choice(
  params$correlation_operator, c(">=", ">"), "params$correlation_operator")
correlation_fdr_threshold <- validate_numeric(
  params$fdr_threshold, "params$fdr_threshold", 0, 1)
correlation_fdr_operator <- validate_choice(
  params$fdr_operator, c("<", "<="), "params$fdr_operator")
...
dys_correlation_operator <- if (correlation_operator == ">=") "<=" else "<"
...
correlation_fdr_rule <- paste(
  "padj", correlation_fdr_operator, format_cutoff(correlation_fdr_threshold))
...
if (length(missing_files)) {
  stop("Missing input file(s): ",
       paste(names(files[missing_files]), files[missing_files],
             sep = " = ", collapse = "; "))
}
expected_md5 <- params$expected_signature_file_md5
if (!is.null(expected_md5) && length(expected_md5) &&
...
output_corr_match <- regexec("corr([0-9]{3})$", basename(output_dir),
                             ignore.case = TRUE)
output_corr_parts <- regmatches(basename(output_dir), output_corr_match)[[1]]
if (length(output_corr_parts) == 2L) {
  output_corr_threshold <- as.numeric(output_corr_parts[2]) / 100
  if (!isTRUE(all.equal(output_corr_threshold, correlation_threshold,
                        tolerance = 1e-12))) {
    stop("output_dir suffix ", output_corr_parts[1], " represents ",
         format_cutoff(output_corr_threshold),
         ", but correlation_threshold is ",
         format_cutoff(correlation_threshold),
         ". Rename output_dir or change correlation_threshold.")
  }
}
...
gene_selection_parameters <- tibble(
  parameter = c(
    "signature_file", "correlation_threshold", "correlation_operator",
    "effective_fun_cim_rule", "effective_dys_cim_rule",
    "fdr_threshold", "fdr_operator", "effective_correlation_fdr_rule",
    "expected_fun_cim_genes", "expected_dys_cim_genes",
    "cox_fdr_threshold", "cox_fdr_operator", "output_dir", "parameter_source"
  ),
  value = c(
    as.character(params$signature_file), format_cutoff(correlation_threshold),
    correlation_operator, fun_correlation_rule, dys_correlation_rule,
    format_cutoff(correlation_fdr_threshold), correlation_fdr_operator,
    correlation_fdr_rule,
    if (is.null(expected_fun_cim_genes)) "null" else expected_fun_cim_genes,
    if (is.null(expected_dys_cim_genes)) "null" else expected_dys_cim_genes,
...
```{r gene-sets}
# Correlation-based signature construction
correlation_raw <- readr::read_csv(files[["correlation"]], show_col_types = FALSE,
                                   name_repair = "minimal")
required_corr <- c("gene_id", "r_rb", "padj")
missing_corr  <- setdiff(required_corr, names(correlation_raw))
if (length(missing_corr))
  stop("Correlation file is missing: ", paste(missing_corr, collapse = ", "))
if (!"trajectory" %in% names(correlation_raw))
  correlation_raw$trajectory <- NA_character_
gene_selection_audit <- correlation_raw %>% transmute(
  gene = toupper(trimws(as.character(gene_id))),
  r_rb = safe_numeric(r_rb),
  source_p_adjust = safe_numeric(padj),
  input_trajectory = as.character(trajectory),
  signature = case_when(
    is.finite(r_rb) & r_rb > 0 ~ "Fun-CIM",
    is.finite(r_rb) & r_rb < 0 ~ "Dys-CIM",
    TRUE ~ "NS"
  ),
  directional_correlation_strength = case_when(
    signature == "Fun-CIM" ~  r_rb,
    signature == "Dys-CIM" ~ -r_rb,
    TRUE ~ NA_real_
  ),
  input_trajectory_matches_derived_signature = case_when(
    is.na(input_trajectory) | str_squish(input_trajectory) == "" ~ NA,
    TRUE ~ str_to_lower(str_squish(input_trajectory)) == str_to_lower(signature)
  ),
  passes_correlation = is.finite(directional_correlation_strength) &
    apply_comparator(directional_correlation_strength,
                     correlation_threshold, correlation_operator),
  passes_fdr = is.finite(source_p_adjust) &
    apply_comparator(source_p_adjust,
                     correlation_fdr_threshold, correlation_fdr_operator),
  correlation_threshold_used = correlation_threshold,
  correlation_operator_used  = correlation_operator,
  fdr_threshold_used         = correlation_fdr_threshold,
  fdr_operator_used          = correlation_fdr_operator,
  selected = !is.na(gene) & gene != "" &
    signature %in% c("Fun-CIM", "Dys-CIM") &
    passes_correlation & passes_fdr
)
gene_definitions <- gene_selection_audit %>%
  filter(selected) %>%
  arrange(gene, source_p_adjust, desc(abs(r_rb))) %>%
  distinct(gene, .keep_all = TRUE)
gene_sets <- split(gene_definitions$gene, gene_definitions$signature)
if (!all(c("Fun-CIM", "Dys-CIM") %in% names(gene_sets)))
  stop("Both Fun-CIM and Dys-CIM must remain after gene-set filtering.")
...
if (!is.null(expected_fun_cim_genes) &&
  ...
  stop("Fun-CIM selected-gene count is ", observed_signature_counts[["Fun-CIM"]],
       ...
       ". Confirm the correlation-file version and active thresholds, or set ",
...
gene_selection_verification <- gene_definitions %>%
  group_by(signature) %>%
  summarise(
    selected_genes = n(),
    minimum_selected_abs_r_rb = min(abs(r_rb)),
    maximum_selected_padj     = max(source_p_adjust),
...
if (any(gene_selection_verification$genes_failing_runtime_correlation_rule > 0L) ||
    any(gene_selection_verification$genes_failing_runtime_fdr_rule > 0L))
...
parameter_values <- list(
  ...
  signature_file_md5 = unname(tools::md5sum(files[["correlation"]])),
  expected_signature_file_md5 = params$expected_signature_file_md5,
  correlation_threshold = correlation_threshold,
  correlation_operator = correlation_operator,
  effective_fun_cim_rule = fun_correlation_rule,
  effective_dys_cim_rule = dys_correlation_rule,
  fdr_threshold = correlation_fdr_threshold,
  fdr_operator = correlation_fdr_operator,
  effective_correlation_fdr_rule = correlation_fdr_rule,
