# ============================================================================
# neo_zscore_heatmap_fry.R
# ============================================================================
# Purpose:
#   Z-score heatmap of 19 immune/stress gene sets across CIMIC clusters
#   for the NEO cohort. Statistical testing via limma::fry, NOMINAL p-values
#   (no FDR) — justified by a priori pathway selection.
#   No blocking — one patient per sample (same structure as NKI_SMC).
#   Cluster coding: "1" = Dys-CIM, "2" = Fun-CIM (same as NKI_SMC).
#   GOBP_INFLAMMATORY_CELL_APOPTOTIC_PROCESS is part of the 19-set list.
#
# Inputs:
#   - oncoimmunology_paper/Datasets/NEO/neo_clustered_plot_df.csv
#
# Outputs:
#   - oncoimmunology_paper/Results/neo_zscore_heatmap_fry.png
#   - oncoimmunology_paper/Results/neo_zscore_heatmap_fry_camera_statistics.csv
...
# NEO: "1" = Dys-CIM, "2" = Fun-CIM (same as NKI_SMC)
metadata_cols <- c(
...
# ============================================================================
# STATISTICAL TESTING: limma::fry — NOMINAL p, NO blocking
# ============================================================================
...
message(sprintf("NEO fry: running on %d gene sets", length(fry_gene_sets)))
...
message("✓ Saved: Results/neo_zscore_heatmap_fry.png")
message("  19 gene sets | NEO cohort | fry | nominal p | no block")


# ============================================================================
# EXPORT FRY + CAMERA STATISTICS: NEO HEATMAP
# ============================================================================

neo_heatmap_stats <- make_fry_camera_table(
...
# ---------------------------------------------------------------------------
zscore_summary_neo <- clustered_plot_df %>%
  dplyr::select(CIMIC_Cluster, all_of(genesets_of_interest)) %>%
  tidyr::pivot_longer(
    cols      = -CIMIC_Cluster,
    names_to  = "GeneSet",
...
neo_heatmap_stats <- neo_heatmap_stats %>%
  dplyr::left_join(zscore_summary_neo, by = "GeneSet")

if (anyNA(neo_heatmap_stats$MeanZscore_Cluster1_DysCIM)) {
  warning(
    "Z-score columns are NA for: ",
    paste(
      neo_heatmap_stats$GeneSet[
        is.na(neo_heatmap_stats$MeanZscore_Cluster1_DysCIM)
      ],
      collapse = ", "
    )
  )
...
data.table::fwrite(
  ...
  file = paste0(
    ...
    "neo_zscore_heatmap_fry_camera_statistics.csv"
  ),
  na = "NA"
)

message(
  "Saved: Results/neo_zscore_heatmap_fry_camera_statistics.csv"
)
