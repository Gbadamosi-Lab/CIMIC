...
#   across CIMIC clusters for the NEO cohort.
#
#   Statistical approach: limma::fry, NOMINAL p-values (no FDR).
#   No blocking — one patient per sample.
#   Cluster coding: "1" = Dys-CIM, "2" = Fun-CIM (same as NKI_SMC).
#
#   Five ICD programs: tested together in one fry call.
#   GOBP_INFLAMMATORY_CELL_APOPTOTIC_PROCESS: separate standalone fry call,
#   NOT included in the main pool.
#
# Inputs:
#   - oncoimmunology_paper/Datasets/NEO/neo_clustered_plot_df.csv
#
# Outputs:
#   - Results/neo_icd_programs_fry.png                (faceted, 5 programs)
#   - Results/neo_icd_<Program>_fry.png               (individual x5)
#   - Results/neo_icd_Inflammatory_Cell_Apoptosis_fry.png  (standalone)
#   - Results/neo_icd_programs_fry_camera_statistics.csv
...
message(sprintf("NEO: Loaded %d samples | %d gene columns", nrow(clustered_plot_df), length(gene_cols)))
...
# ============================================================================
# MAIN FRY: five ICD programs — NOMINAL p, no block
# ============================================================================
...
message(sprintf("NEO main fry: %d gene sets", length(fry_gene_sets)))
...
if (length(inflam_idx[[1]]) >= 5) {
  ...
  inflam_signif <- case_when(
    ...
    inflam_p <= 0.01   ~ "**",
    inflam_p <= 0.05   ~ "*",
    inflam_p <  0.10   ~ sprintf("p = %.3g", inflam_p),
    TRUE               ~ "ns"
  )
  message(sprintf("NEO standalone fry [%s]: PValue = %.4f (%s)",
...
p_facet <- ggplot(
  ...
  labs(
    title = "ICD Programs — NEO (fry, nominal p)",
    y     = "Z-score of\nProgram Activation",
    x     = NULL
  ) +
...
ggsave(
  "oncoimmunology_paper/Results/neo_icd_programs_fry.png",
  plot   = p_facet,
  width  = 12,
  height = 14,
  dpi    = 300,
...
message("✓ Saved: Results/neo_icd_programs_fry.png")
...
make_individual_boxplots <- function(sig_df) {
  for (prog in unique(program_scores_all$Program)) {
    ...
    out_file <- sprintf(
      "oncoimmunology_paper/Results/neo_icd_%s_fry.png",
      gsub(" ", "_", prog)
    )
...
ggsave(
  "oncoimmunology_paper/Results/neo_icd_Inflammatory_Cell_Apoptosis_fry.png",
  plot   = p_inflam,
  width  = 7,
  height = 7,
  dpi    = 300,
...
message("✓ Saved: Results/neo_icd_Inflammatory_Cell_Apoptosis_fry.png")
message("  [Standalone fry — excluded from main pool]")



# ============================================================================
# EXPORT FRY + CAMERA STATISTICS: NEO ICD PROGRAMS
# ============================================================================

neo_icd_main_stats <- make_fry_camera_table(
...
neo_inflammatory_set <- list(
  GOBP_INFLAMMATORY_CELL_APOPTOTIC_PROCESS =
    intersect(inflam_apop_genes, rownames(expr_matrix))
)

neo_inflammatory_stats <- make_fry_camera_table(
  expr_matrix   = expr_matrix,
  gene_sets     = neo_inflammatory_set,
  design        = design,
  contrast      = contrast,
  cohort_type   = "human",
...
neo_icd_all_stats <- dplyr::bind_rows(
  neo_icd_main_stats,
  neo_inflammatory_stats
)
...
zscore_summary <- icd_zscore_long %>%
  ...
  # NEO coding: "1" = Dys-CIM, "2" = Fun-CIM
  dplyr::mutate(
...
neo_icd_all_stats <- neo_icd_all_stats %>%
  dplyr::left_join(zscore_summary, by = "GeneSet")

if (anyNA(neo_icd_all_stats$MeanZscore_Cluster1_DysCIM)) {
  warning(
    "Z-score columns are NA for: ",
    paste(
      neo_icd_all_stats$GeneSet[
        is.na(neo_icd_all_stats$MeanZscore_Cluster1_DysCIM)
      ],
      collapse = ", "
    )
  )
...
data.table::fwrite(
  ...
  file = paste0(
    ...
    "neo_icd_programs_fry_camera_statistics.csv"
  ),
  na = "NA"
)

message(
  "Saved: Results/neo_icd_programs_fry_camera_statistics.csv"
)