...
#          Fun‑CIM) based on the limma‑derived correlation results.
# Inputs:  oncoimmunology_paper/Results/nki_smc_correlation_analysis.csv
...
run_ora <- function(gene_list, set_name) {
  ...
  # Save plot
  plot_file <- file.path(output_dir, paste0("fig2c_", set_name, "_ORA.png"))
  ggsave(plot_file, plot = p, width = 14, height = 10, dpi = 300)
  message("Saved plot: ", plot_file)

  # Save ORA table
  csv_file <- file.path(output_dir, paste0("fig2c_", set_name, "_ORA.csv"))
