---
title: "NKI CIM RFS Master Analysis"
subtitle: "Paired gene-expression change and ssGSEA signature-change analyses"
...
output:
  html_document:
    ...
    theme: cosmo
params:
  output_dir: "NKI_CIM_RFS_master_results"

  # Input filenames expected inside data/.
  survival_file: "NKI_survival_data.xlsx"
  expression_file: "nki_smc_combine_TPM_lognorm.csv"
  signature_file: "nki_smc_correlation_analysis.csv"

  # Shared CIM gene-selection settings used by BOTH modules.
...
# During knitting, rmarkdown creates `params` from the YAML header (including
# any values supplied through "Knit with Parameters"). During interactive
# chunk execution, an old `params` object can remain in the Global Environment.
# Reload the active RMD's YAML in that situation so edited cutoffs take effect.
analysis_parameter_source <- "parameters supplied by rmarkdown during knit"
analysis_rmd_path <- knitr::current_input()

if (!isTRUE(getOption("knitr.in.progress"))) {
  active_rmd_path <- tryCatch(
    {
  ...
  if (!nzchar(active_rmd_path) || !file.exists(active_rmd_path)) {
    stop(
      "Interactive execution could not identify the active RMD. ",
      "Knit the complete document so the YAML parameters are applied."
    )
  }

  yaml_configuration <- rmarkdown::yaml_front_matter(active_rmd_path)
  if (!is.list(yaml_configuration$params)) {
    stop("The active RMD does not contain a valid YAML params section.")
  }

  params <- yaml_configuration$params
  analysis_rmd_path <- active_rmd_path
  analysis_parameter_source <- paste0(
    "YAML reloaded for interactive execution: ", basename(active_rmd_path)
  )
}
```
...
rmd_input <- analysis_rmd_path
rmd_dir <- if (length(rmd_input) == 1L && nzchar(rmd_input)) {
  dirname(normalizePath(rmd_input, winslash = "/", mustWork = FALSE))
} else {
  normalizePath(getwd(), winslash = "/", mustWork = FALSE)
}

candidate_data_dirs <- unique(c(
  file.path(rmd_dir, "data"),
  file.path(getwd(), "data")
))

required_input_names <- c(
  survival = as.character(params$survival_file),
  expression = as.character(params$expression_file),
  signature = as.character(params$signature_file)
)

data_dir_is_complete <- vapply(
  candidate_data_dirs,
  function(path) all(file.exists(file.path(path, required_input_names))),
  logical(1)
)

if (!any(data_dir_is_complete)) {
  stop(
    "Could not find a data/ folder containing all required files:\n",
    paste(required_input_names, collapse = "\n"),
    "\n\nFolders checked:\n",
    paste(candidate_data_dirs, collapse = "\n"),
    "\n\nFor the GitHub repository, place the input files in a data/ folder ",
    "beside this RMD or knit from the repository root."
  )
}

data_dir <- candidate_data_dirs[which(data_dir_is_complete)[1]]

input_files <- c(
  survival = file.path(data_dir, required_input_names[["survival"]]),
  expression = file.path(data_dir, required_input_names[["expression"]]),
  signature = file.path(data_dir, required_input_names[["signature"]])
)
...
required_signature_columns <- c("gene_id", "r_rb", "p_value", "padj", "trajectory")
missing_signature_columns <- setdiff(required_signature_columns, names(signature_raw))
if (length(missing_signature_columns) > 0) {
  stop(
    "Signature file is missing required column(s): ",
    paste(missing_signature_columns, collapse = ", ")
  )
}

correlation_data <- signature_raw %>%
  transmute(
    gene = stringr::str_trim(as.character(gene_id)),
    r_rb = suppressWarnings(as.numeric(r_rb)),
    correlation_p_value = suppressWarnings(as.numeric(p_value)),
    correlation_padj = suppressWarnings(as.numeric(padj)),
    input_trajectory = case_when(
      str_to_lower(str_squish(as.character(trajectory))) == "fun-cim" ~ "Fun-CIM",
      str_to_lower(str_squish(as.character(trajectory))) == "dys-cim" ~ "Dys-CIM",
      str_to_lower(str_squish(as.character(trajectory))) == "ns" ~ "NS",
      TRUE ~ NA_character_
    )
  ) %>%
...
if (anyNA(correlation_data$input_trajectory)) {
  stop("Unexpected trajectory labels were found; expected Fun-CIM, Dys-CIM, or NS.")
}
if (any(!is.finite(correlation_data$r_rb)) ||
    any(!is.finite(correlation_data$correlation_padj))) {
  stop("Missing or non-finite r_rb/padj values were found in the correlation file.")
}

validate_unit_cutoff <- function(value, parameter_name) {
  value <- suppressWarnings(as.numeric(value))
  if (length(value) != 1L || !is.finite(value) || value < 0 || value > 1) {
    stop(parameter_name, " must be one finite numeric value from 0 to 1.")
  }
  value
}
...
correlation_threshold <- validate_unit_cutoff(
  params$correlation_threshold, "params$correlation_threshold"
)
correlation_operator <- str_trim(as.character(params$correlation_operator))
if (length(correlation_operator) != 1L ||
    !correlation_operator %in% c(">=", ">")) {
  stop("params$correlation_operator must be either '>=' or '>'.")
}

correlation_fdr_threshold <- validate_unit_cutoff(
  params$fdr_threshold, "params$fdr_threshold"
)
correlation_fdr_operator <- str_trim(as.character(params$fdr_operator))
if (length(correlation_fdr_operator) != 1L ||
    !correlation_fdr_operator %in% c("<", "<=")) {
  stop("params$fdr_operator must be either '<' or '<='.")
}

cox_fdr_threshold <- validate_unit_cutoff(
  params$cox_fdr_threshold, "params$cox_fdr_threshold"
)

correlation_fdr_stage_label <- sprintf(
  "Correlation padj %s %s",
  correlation_fdr_operator, format_cutoff(correlation_fdr_threshold)
)
correlation_stage_label <- sprintf(
  "FDR-qualified + directional |r_rb| %s %s",
  correlation_operator, format_cutoff(correlation_threshold)
)

dys_correlation_operator <- if (
  identical(correlation_operator, ">=")
) "<=" else "<"
fun_correlation_rule <- paste(
  "r_rb", correlation_operator, format_cutoff(correlation_threshold)
)
dys_correlation_rule <- paste(
  "r_rb", dys_correlation_operator,
  paste0("-", format_cutoff(correlation_threshold))
)
correlation_fdr_rule <- paste(
  "padj", correlation_fdr_operator,
  format_cutoff(correlation_fdr_threshold)
)

analysis_parameters <- tibble::tibble(
  parameter = c(
    "correlation_padj_threshold",
    "correlation_padj_operator",
    "absolute_rank_biserial_correlation_threshold",
    "directional_correlation_operator",
    "effective_fun_cim_rule",
    "effective_dys_cim_rule",
    "effective_correlation_fdr_rule",
    "signature_file",
    "parameter_source",
    "cox_fdr_threshold",
  ...
  value = c(
    format_cutoff(correlation_fdr_threshold),
    correlation_fdr_operator,
    format_cutoff(correlation_threshold),
    correlation_operator,
    fun_correlation_rule,
    dys_correlation_rule,
    correlation_fdr_rule,
    as.character(params$signature_file),
    analysis_parameter_source,
    format_cutoff(cox_fdr_threshold),
    as.character(isTRUE(params$run_firth_sensitivity))
  )
)

readr::write_csv(
  analysis_parameters,
  file.path(qc_dir, "analysis_parameters.csv")
)

cat(
  "Gene-selection filters parsed from YAML:\n",
  "  Fun-CIM: ", fun_correlation_rule, "\n",
  "  Dys-CIM: ", dys_correlation_rule, "\n",
  "  Correlation FDR: ", correlation_fdr_rule, "\n",
  sep = ""
)

signature_filter_audit <- correlation_data %>%
  mutate(
    directional_correlation_strength = case_when(
      direction == "Fun-CIM" ~ r_rb,
      direction == "Dys-CIM" ~ -r_rb,
      TRUE ~ NA_real_
    ),
    passes_expected_direction = case_when(
      direction == "Fun-CIM" ~ r_rb > 0,
      direction == "Dys-CIM" ~ r_rb < 0,
      TRUE ~ FALSE
    ),
    input_trajectory_matches_derived_direction =
      input_trajectory == direction,
    passes_correlation_fdr = apply_comparator(
      correlation_padj, correlation_fdr_threshold, correlation_fdr_operator
    ),
    passes_correlation_threshold = apply_comparator(
      directional_correlation_strength,
      correlation_threshold,
      correlation_operator
    ),
    correlation_threshold_used = correlation_threshold,
    correlation_operator_used = correlation_operator,
    correlation_fdr_threshold_used = correlation_fdr_threshold,
    correlation_fdr_operator_used = correlation_fdr_operator,
    selected_for_analysis =
      direction %in% c("Fun-CIM", "Dys-CIM") &
      passes_expected_direction &
      passes_correlation_fdr &
...
signature <- signature_filter_audit %>%
  filter(selected_for_analysis)

selection_verification <- signature %>%
  group_by(direction) %>%
  summarise(
    selected_genes = n(),
    minimum_selected_abs_r_rb = min(abs(r_rb)),
    maximum_selected_padj = max(correlation_padj),
    genes_failing_runtime_correlation_rule = sum(
      !apply_comparator(
        directional_correlation_strength,
        correlation_threshold,
        correlation_operator
    ...
    genes_failing_runtime_fdr_rule = sum(
      !apply_comparator(
        correlation_padj,
        correlation_fdr_threshold,
        correlation_fdr_operator
      )
    ),
    .groups = "drop"
  ...
  tidyr::complete(
    direction = c("Fun-CIM", "Dys-CIM"),
    fill = list(
      selected_genes = 0L,
      genes_failing_runtime_correlation_rule = 0L,
      genes_failing_runtime_fdr_rule = 0L
    )
  )
...
readr::write_csv(
  signature %>%
    arrange(direction, desc(abs(r_rb)), gene),
  file.path(qc_dir, "selected_signature_genes.csv")
)

if (any(selection_verification$genes_failing_runtime_correlation_rule > 0L) ||
    any(selection_verification$genes_failing_runtime_fdr_rule > 0L)) {
  stop("Internal error: selected genes do not satisfy the runtime YAML filters.")
}

cat("Selected genes after applying the runtime YAML filters:\n")
print(selection_verification %>% select(direction, selected_genes))

if (!nrow(signature)) {
  stop(
    "No Fun-CIM or Dys-CIM genes passed the selected filters: padj ",
    correlation_fdr_operator, " ", format_cutoff(correlation_fdr_threshold),
    " and directional |r_rb| ", correlation_operator, " ",
    format_cutoff(correlation_threshold), "."
  )
}

duplicate_signature_genes <- correlation_data %>%
  count(gene, name = "n_rows") %>%
  filter(n_rows > 1)

readr::write_csv(duplicate_signature_genes,
                 file.path(qc_dir, "duplicated_signature_genes.csv"))

if (nrow(duplicate_signature_genes) > 0) {
  stop(
    "Duplicated genes were found in the signature file. Review: ",
    file.path(qc_dir, "duplicated_signature_genes.csv")
  )
}

signature_summary <- bind_rows(
  correlation_data %>%
    filter(direction %in% c("Fun-CIM", "Dys-CIM")) %>%
    count(direction, name = "n_genes") %>%
    mutate(stage = "Direction assigned from r_rb sign"),
  correlation_data %>%
    filter(
      direction %in% c("Fun-CIM", "Dys-CIM"),
      apply_comparator(
        correlation_padj, correlation_fdr_threshold,
        correlation_fdr_operator
      )
    ) %>%
    count(direction, name = "n_genes") %>%
    mutate(stage = correlation_fdr_stage_label),
  signature %>%
    count(direction, name = "n_genes") %>% mutate(stage = correlation_stage_label)
) %>%
  select(stage, direction, n_genes) %>%
  arrange(factor(stage, levels = c(
    "Direction assigned from r_rb sign", correlation_fdr_stage_label,
    correlation_stage_label
  )), direction)

readr::write_csv(signature_summary, file.path(qc_dir, "signature_filtering_stage_counts.csv"))
...
nki_expression_long <- expression_data %>%
  ...
  select(gene, all_of(nki_sample_metadata$sample_id)) %>%
  pivot_longer(
    cols = -gene,
    names_to = "sample_id",
  ...
  left_join(
    nki_sample_metadata %>% select(sample_id, patient_id, timepoint),
    by = "sample_id"
  ) %>%
...
gene_changes <- nki_expression_long %>%
  select(gene, patient_id, timepoint, expression) %>%
...
gene_change_matrix <- gene_changes %>%
  select(gene, patient_id, delta_expression) %>%
...
kable(
  direction_summary %>%
    select(signature, evidence_subset, genes_tested,
...
```{r rfs-forest-plots, fig.width=2.85, fig.height=4.00}
export_forest <- function(signature_name) {
  ...
  readr::write_csv(
    selected %>%
      select(signature_direction, gene, survival_direction, coefficient,
...
prepare_heatmap_matrix <- function(selected_genes) {
  gene_changes %>%
    ...
    select(gene, patient_id, delta_z) %>%
...
save_change_heatmap <- function(
  ...
  row_annotation <- signature %>%
    ...
    select(gene, Signature = direction) %>%
    column_to_rownames("gene")
...
```{r ssgsea-publication-forest, fig.width=4.25, fig.height=2.15}
if (
