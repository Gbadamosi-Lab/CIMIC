...
#          Fun‑CIM) based on the Neo dataset correlation results.
# Inputs:  oncoimmunology_paper/Results/neo_correlation_analysis.csv
# Outputs: ORA plots and CSV files saved to oncoimmunology_paper/Results/
# ============================================================================
...
# ---------------------------------------------------------------------------
# Define gene sets for each trajectory
# ---------------------------------------------------------------------------
# Dys‑CIM (C1) – negative correlation (r_rb <= -cutoff)
# Fun‑CIM (C2) – positive correlation (r_rb >= cutoff)

dys_genes_df <- cor_df %>%
  filter(trajectory == "Dys-CIM", padj < padj_cutoff, r_rb <= -r_rb_cutoff)

fun_genes_df <- cor_df %>%
  filter(trajectory == "Fun-CIM", padj < padj_cutoff, r_rb >= r_rb_cutoff)
...
run_ora <- function(gene_list, set_name) {
  ...
  # Save plot
  plot_file <- file.path(output_dir, paste0("fig4_neo_", set_name, "_ORA.png"))
  ggsave(plot_file, plot = p, width = 14, height = 10, dpi = 300)
  message("Saved plot: ", plot_file)

  # Save ORA table
  csv_file <- file.path(output_dir, paste0("fig4_neo_", set_name, "_ORA.csv"))
...
message("ORA analysis for Neo dataset completed.")
