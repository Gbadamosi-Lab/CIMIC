# ============================================================================
# pre_treatment_correlation_analysis.R
# ============================================================================
...
#            (1) the treatment-induced delta (post - pre) and the
#               pre-treatment profile ("Delta vs Pre").
#          This script omits the "Pre vs Post" comparison.
#
...
#
# Outputs:
#   oncoimmunology_paper/Results/pre_treatment_correlation_nki_smc.csv
#   oncoimmunology_paper/Results/pre_treatment_correlation_neo.csv
#   oncoimmunology_paper/Results/pre_treatment_correlation_tnbc_cl.csv
#   oncoimmunology_paper/Results/pre_treatment_correlation_nki_smc.png
#   oncoimmunology_paper/Results/pre_treatment_correlation_neo.png
#   oncoimmunology_paper/Results/pre_treatment_correlation_tnbc_cl.png
# ============================================================================

library(data.table)   # fast CSV reading
library(dplyr)        # data manipulation
library(ggplot2)      # plotting
library(rstatix)       # paired Wilcoxon test (kept for potential use)
...
# Returns a long data.frame (one row per sample) with BH‑adjusted p‑values.
# ---------------------------------------------------------------------------
compute_delta_pre_correlation <- function(pre_mat, post_mat, sample_ids) {
...
# ---------------------------------------------------------------------------
# 1. Boxplot of rho values with reference lines at +/-0.3 and annotations for
#    average rho and the proportion of samples exceeding those thresholds.
#    Optional `shape_var` can be provided (e.g., cell line) to map point shapes.
build_boxplot <- function(corr_df, title, shape_var = NULL) {
  corr_df$comparison <- factor(corr_df$comparison, levels = "Delta vs Pre")

  # Percentages of moderate/strong correlations
  perc_pos <- mean(corr_df$rho > 0.3, na.rm = TRUE) * 100
  perc_neg <- mean(corr_df$rho < -0.3, na.rm = TRUE) * 100
  perc_label <- sprintf("rho > 0.3: %.1f%%   rho < -0.3: %.1f%%", perc_pos, perc_neg)

  avg_rho <- mean(corr_df$rho, na.rm = TRUE)

  p <- ggplot(corr_df, aes(x = comparison, y = rho)) +
    geom_boxplot(outlier.shape = NA, colour = "black", fill = "#8DA0CB", width = 0.5, alpha = 0.75)

  # Add jittered points; optionally map shape if a valid column is supplied
  if (!is.null(shape_var) && shape_var %in% colnames(corr_df)) {
    p <- p + geom_jitter(aes(shape = .data[[shape_var]]), width = 0.15, size = 2.5, colour = "black", alpha = 0.75) +
      scale_shape_manual(values = 0:8)
  } else {
    p <- p + geom_jitter(width = 0.15, size = 2.5, colour = "black", alpha = 0.75)
  }

  p +
    geom_hline(yintercept = 0, linetype = "dotted", colour = "black") +
    geom_hline(yintercept = 0.3, linetype = "dotted", colour = "grey50") +
    geom_hline(yintercept = -0.3, linetype = "dotted", colour = "grey50") +
    ...
    # Move annotation down a bit to avoid cutoff
    annotate("text", x = 1, y = 0.95, label = sprintf("avg \u03C1 = %.3f", avg_rho), size = 7, fontface = "bold", vjust = 0) +
    annotate("text", x = 1, y = 0.8, label = perc_label, size = 7, fontface = "bold", vjust = 0)
}
...
nki_corr <- compute_delta_pre_correlation(nki_pre_mat, nki_post_mat, nki_prefixes)

write.csv(nki_corr, file.path(results_dir, "pre_treatment_correlation_nki_smc.csv"), row.names = FALSE)

nki_avg <- average_correlation(nki_corr)
write.csv(nki_avg, file.path(results_dir, "pre_treatment_correlation_nki_smc_avg.csv"), row.names = FALSE)

nki_plot <- build_boxplot(nki_corr, "NKI_SMC")
ggsave(file.path(results_dir, "pre_treatment_correlation_nki_smc.png"),
...
neo_corr <- compute_delta_pre_correlation(neo_pre_mat, neo_post_mat, neo_pre_prefix)

write.csv(neo_corr, file.path(results_dir, "pre_treatment_correlation_neo.csv"), row.names = FALSE)

neo_avg <- average_correlation(neo_corr)
write.csv(neo_avg, file.path(results_dir, "pre_treatment_correlation_neo_avg.csv"), row.names = FALSE)

neo_plot <- build_boxplot(neo_corr, "NEO")
ggsave(file.path(results_dir, "pre_treatment_correlation_neo.png"),
       plot = neo_plot, width = 6, height = 6, dpi = 300)

# ============================================================================
# TNBC_CL_Epirubicin: "_DMSO_R#" = pre-treatment (vehicle),
# "_EPI_R#" = post-treatment (epirubicin). The DMSO
# replicates are averaged per cell line into a single shared baseline; each
...
tnbc_corr <- compute_delta_pre_correlation(tnbc_pre_mat_expanded, tnbc_epi_mat, tnbc_epi_cols)
tnbc_corr$cell_line <- sub("_EPI_R[0-9]+$", "", tnbc_corr$sample_id)

write.csv(tnbc_corr, file.path(results_dir, "pre_treatment_correlation_tnbc_cl.csv"), row.names = FALSE)

# Save average correlation per comparison for TNBC_CL_Epirubicin
tnbc_avg <- average_correlation(tnbc_corr)
write.csv(tnbc_avg, file.path(results_dir, "pre_treatment_correlation_tnbc_cl_avg.csv"), row.names = FALSE)

tnbc_plot <- build_boxplot(tnbc_corr, "TNBC_CL_Epirubicin", shape_var = "cell_line")
ggsave(file.path(results_dir, "pre_treatment_correlation_tnbc_cl.png"),
