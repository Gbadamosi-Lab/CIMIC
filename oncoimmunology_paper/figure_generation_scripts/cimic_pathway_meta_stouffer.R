...
load_one <- function(family, dataset) {
  ...
  dplyr::bind_rows(lapply(TESTS, function(tst) {
    ...
    fdr <- suppresswarnings(as.numeric(d[[paste0(tst, "_FDR")]]))
    dir <- as.character(d[[paste0(tst, "_Direction")]])
    cbind(base, data.frame(test = tst,
                           p_value = p,
                           p_adj   = fdr,
                           direction = dir,
                           stringsAsFactors = FALSE))
...
long <- long %>%
  dplyr::mutate(
    # Use FDR-adjusted p-values for the Stouffer meta-analysis.
    p_os = ifelse(direction == "Up", p_adj / 2, 1 - p_adj / 2),
