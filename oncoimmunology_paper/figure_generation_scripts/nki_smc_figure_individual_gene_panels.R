# ============================================================================
# nki_smc_figure_individual_gene_panels.R
# ============================================================================
# Purpose:
#   Per-gene two-panel summary across CIMIC trajectories:
#
#     Left:
...
#
# Output:
#   oncoimmunology_paper/Results/figure_gene_panel_<GENE>.png
#
# Dependencies:
...
calculate_avg_mhc1 <- function(df) {
  # Expected column names (case‑sensitive). If any are missing, throw an error.
  ...
  # Extract the three columns and compute row‑wise mean, ignoring NA values.
  mhc_matrix <- as.matrix(df[required_cols])
  # `rowMeans` with `na.rm = TRUE` returns NaN if all values are NA; convert
  # those to NA for consistency.
...
output_dir <- "oncoimmunology_paper/Results/nki_smc_individual_gene_panels"
...
for (gene in genes_of_interest) {
  ...
  output_file <- file.path(
    output_dir,
    sprintf(
      "nki_smc_figure_gene_panel_%s.png",
      gene
    )
  )
