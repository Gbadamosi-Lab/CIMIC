# fig2c_hallmark_ora.R
# ============================================================================
# Purpose: Run Over‑Representation Analysis (ORA) on the Hallmark gene sets
#          from MSigDB using the same Dys‑CIM and Fun‑CIM gene lists that are
#          used for the GO‑BP analysis in fig2c_corr_ora.R.
...
# ---------------------------------------------------------------------------
# Load correlation results (same input as fig2c_corr_ora.R)
# ---------------------------------------------------------------------------
...
# Output directory (same as GO analysis)
...
run_hallmark_ora <- function(gene_list, set_name) {
  ...
  # Save plot
  plot_file <- file.path(output_dir, paste0("fig2c_", set_name, "_Hallmark_ORA.png"))
  ggsave(plot_file, plot = p, width = 14, height = 10, dpi = 300)
  message("Saved plot: ", plot_file)

  # Save enrichment table
  csv_file <- file.path(output_dir, paste0("fig2c_", set_name, "_Hallmark_ORA.csv"))
...
message("Hallmark ORA analysis completed.")
